	...
	HTMLfive.HTML = {
		...
		add: function(AssetName,AssetHTML,options={'fadein':false,'fadeout':false,'shownow':false}) {	// add an element to the DOM
			if (!this.Cache[AssetName]) {
				...
				myHTML.style.top = options.top + "px";										// position the HTML
				myHTML.style.left = options.left + "px";
				myHTML.style.right = options.right + "px";
				myHTML.style.bottom = options.bottom + "px";
				myHTML.style.width = options.width + "px";
				myHTML.style.height = options.height + "px";
		...
		edit: function(AssetName,AssetHTML,options={}) {
			...
			if (curHTML) {
				...
				if(options.top != null) curHTML.style.top = options.top + "px";				// position the HTML
				if(options.left != null) curHTML.style.left = options.left + "px";
				if(options.right != null) curHTML.style.right = options.right + "px";
				if(options.bottom != null) curHTML.style.bottom = options.bottom + "px";
				if(options.width != null) curHTML.style.width = options.width + "px";
				if(options.height != null) curHTML.style.height = options.height + "px";
			}
		},
